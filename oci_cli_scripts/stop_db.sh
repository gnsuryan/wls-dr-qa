...
if [ ! -z "$DB_OCID" ]; then
    echo "Database ${DB_NAME} with $DB_OCID is already in status STOPPED"
    exit 0
fi
...
echo "Stopping the database $DB_NAME with OCID $DB_OCID ..."
...
do
    ...
    then
        echo "Database is not stopped even after 5 mins !!"
        exit 1
    fi
    echo "Waiting the database to be completely stopped..."
    sleep 10s
    DB_OCID=$(eval $(echo "$CMD | grep ocid | sed 's/\"//g' | sed 's/ //g'"))
    if [ ! -z "$DB_OCID" ];
    then
        echo "Database is now completely stopped!!!"
    fi
done
